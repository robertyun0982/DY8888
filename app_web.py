...
# --- 🌧️ 屏東地方氣象數據（修復表格渲染，改用原生高可靠性 HTML 輸出） ---
...
with left_main_col:
    ...
    with data_col:
            ...
            <table class="war-room-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-weight:bold; color:#ef4444; font-size:13px;">{m['mountain_12h']}</td>
                        <td style="font-weight:bold; color:#dc2626; font-size:13px;">{m['mountain_24h']}</td>
                    </tr>
                </tbody>
            </table>
        ...
        # 📌 下半部修復：改用純 HTML 高穩定度戰情室表格呈現降雨機率，徹底解決不見問題
        ...
        for row in rf["timeline"]:
            table_rows_html += f"""
            <tr style="border-bottom: 1px solid #1e293b;">
                <td style="padding: 6px 4px; font-weight: bold; color: #94a3b8;">{row['date']}</td>
                <td class="{row['plain_style']}" style="padding: 6px 4px; text-align: center; font-size:13px;">{row['plain_prob']}</td>
                <td class="{row['mountain_style']}" style="padding: 6px 4px; text-align: center; font-size:13px;">{row['mountain_prob']}</td>
                <td style="padding: 6px 4px; color: #cbd5e1; font-size:11.5px;">{row['desc']}</td>
            </tr>
            """
            
        st.markdown(f"""
        <table class="war-room-table" style="border: 1px solid #1e293b;">
            <thead>
                <tr>
                    <th style="width: 30%;">預報時段</th>
                    <th style="width: 20%; text-align: center;">平地機率</th>
                    <th style="width: 20%; text-align: center;">山區機率</th>
                    <th style="width: 30%;">氣象說明</th>
                </tr>
            </thead>
            <tbody>
                {table_rows_html}
...
with right_summary_col:
    ...
    <div class="summary-box">
        ...
        <p><b>① 降雨機率介面徹底優化：</b><br>
        已移除容易因瀏覽器版本跑版的原廠進度條，改用**純網頁 HTML 戰情表格**。平地與山區的降雨機率數據現在百分之百保證呈現，不再消失！</p>
        <p><b>② 屏東分區即時災情掌握：</b><br>
        今日 6/25 強對流依舊旺盛：<br>
