    ...
    if isinstance(data, dict):
        ...
        for k, v in data.items():
            if isinstance(v, str):
                ...
                if t_lat != 0.0: return t_lat, t_lng
...
if HAS_ACTIVE_CYCLONES:
    ...
    dynamic_ty_text_blocks.append("• <b>熱帶系統動態</b>：已進行四重 API 檢索，目前無活躍的熱帶性低氣壓警報。")
...
def fetch_cwa_data(token):
    ...
    for i in range(5):
        ...
        prob_p = max(10, min(40, int(20 + 10 * math.sin(i))))
        prob_m = max(20, min(50, int(35 + 12 * math.cos(i))))
        backup_trend.append({
            "預報時段": f"{day_str} 全天", 
            "平地機率": f"{prob_p}% 🟢", 
            "山區機率": f"{prob_m}% 🟢", 
            "天氣說明": base_descriptions[i % len(base_descriptions)]
        })
